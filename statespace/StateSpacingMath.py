...
class StateSpaceMathNumpyV1(MathProtocol):
...
class StateSpaceMathNumpyV2(MathProtocol):
...
class StateSpaceMathNumpyV3(MathProtocol):
    ...
    def _mm3(M1: np.ndarray, M2: np.ndarray, M3: np.ndarray, dtype=np.float64):
        ...
        Returns:
            M1M2M3: product of 3 matrices
        """
        return np.einsum('mn, nk, kp -> mp', M1, M2, M3)
    ...
    Returns: ev
        M1M2M3: batch matrix multiplication of 3 matrices
    """
        return np.einsum('Bmn, Bnk, Bkp -> mp', M1, M2, M3)
